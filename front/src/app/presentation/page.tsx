...
const Presentation = () => {
  ...
  return (
    <div>
        ...
        <p className='text-center'>Interest est un réseau social fondé sur les centres d'intérêt. Notre priorité est de vous offrir du contenu exclusivement lié à vos passions, sans publicité, juste du partage entre passionnés. </p>
        ...
        <div className=' flex flex-col gap-5 text-center bg-primary py-4'>
        <p> Pour plus d'informations vous pouvez consulter <span className='text-white hover:underline cursor-pointer' onClick={()=>{
            router.push('/confidentialiter')
        }}>la confidentialitée du site</span> et <span className='text-white hover:underline cursor-pointer' onClick={()=>{
            router.push('/cgu')
        }}>les conditions d'utilisations</span> </p>
        <p> Pour toute question ou réclamation merci de vous diriger vers <span className='text-white hover:underline cursor-pointer' onClick={()=>{
            router.push('/contact')
        }}>la page contact</span></p>
        </div>
    </div>
  )
}
